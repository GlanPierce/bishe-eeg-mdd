...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="10-fold CV for dual-graph multiband fusion (PCC + PLV theta/alpha/beta)")
    ...
    p.add_argument(
        "--out-path",
        type=Path,
        default=Path("outputs/metrics/gnn_dualgraph_multiband_pcc_plv_cv10_metrics.json"),
    )
    p.add_argument("--experiment-name", type=str, default="gnn_dualgraph_multiband_pcc_plv_cv10")
    return p.parse_args()
...
def main() -> int:
    ...
    payload = {
        ...
        "model": "DualGraphMultiBandFusionModel",
        "device": str(device),
        "params": {
            "folds": args.folds,
            "epochs": args.epochs,
