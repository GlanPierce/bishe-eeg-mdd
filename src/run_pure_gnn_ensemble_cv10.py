...
def train_spatiotemporal_one(
    ...
    if bool(cfg.get("refit_full_train", False)):
        np.random.seed(int(seed))
        torch.manual_seed(int(seed))
        full_ds = spatio_mod.TemporalDualGraphDataset(train_items + val_items)
        full_loader = torch.utils.data.DataLoader(full_ds, batch_size=int(cfg["batch_size"]), shuffle=True, collate_fn=spatio_mod.collate_fn)
        full_model = spatio_mod.DualGraphMultiBandSpatioTemporalModel(
            in_dim=in_dim,
            hidden_dim=int(cfg["hidden_dim"]),
            temporal_hidden=int(cfg["temporal_hidden"]),
            dropout=float(cfg["dropout"]),
        ...
        full_opt = torch.optim.Adam(full_model.parameters(), lr=float(cfg["lr"]), weight_decay=float(cfg["weight_decay"]))
        for _ in range(int(best_epoch)):
            _ = spatio_mod.train_one_epoch(full_model, full_loader, full_opt, device)
        _, p_test_refit, _, _ = spatio_mod.predict(full_model, test_loader, device)
        result["p_test"] = p_test_refit
    return result
...
def main() -> int:
    ...
    for seed in seeds:
        ...
        for fold_no, (trainval_idx, test_idx) in enumerate(skf.split(np.arange(len(spatio_items)), y_spatio), start=1):
            ...
            y_val = res_flat_static["y_val"]
            y_test = res_flat_static["y_test"]
            weights_sel, thr_sel, val_acc = select_weights_threshold(
                y_val=y_val,
                prob_flat_static=res_flat_static["p_val"],
                prob_st_base=res_st_base["p_val"],
                prob_flatwin=res_flatwin["p_val"],
            ...
            prob_test = (
                (weights_sel[0] * res_flat_static["p_test"])
                + (weights_sel[1] * res_st_base["p_test"])
                + (weights_sel[2] * res_flatwin["p_test"])
            )
