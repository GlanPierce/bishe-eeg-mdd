...
### A1. Clean benchmark results
These are the lower-overfitting full-dataset results. They do not use dataset-tuned subject-level repair rules.
...
### A2. Benchmark-tuned artifact-aware repair variant
This keeps all `61` subjects, but adds subject-level repair rules tuned against this benchmark. It should be reported separately from the clean benchmark because it has higher benchmark-overfitting risk.

| Method | Script | Accuracy | Balanced Accuracy | F1 | ROC-AUC |
|---|---|---:|---:|---:|---:|
| Explicit region + temporal pure GNN (`ExplicitRegionTemporalWeightedStarGNN`) + targeted artifact repair | `src/run_explicit_region_temporal_summary_node_gnn_5x10.py` with targeted repair flags | **`0.9343 +/- 0.0012`** | **`0.9283 +/- 0.0041`** | **`0.9412 +/- 0.0021`** | **`0.9581 +/- 0.0190`** |

### Artifact-repair flags for A2
- `--c 0.25`
...
- If the target is the main thesis benchmark on the full dataset, default to the clean `0.9143` line from `run_explicit_region_temporal_summary_node_gnn_5x10.py` without targeted repair flags.
- If you report the `0.9343` full-dataset result, label it explicitly as a benchmark-tuned artifact-aware repair variant and keep it separate from the clean benchmark line.
